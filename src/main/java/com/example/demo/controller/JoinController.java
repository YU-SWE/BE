...
public class JoinController {
    ...
    public ResponseEntity<?> join(@Valid @RequestBody JoinForm joinForm, BindingResult bindingResult) {
        ...
        if (!joinForm.getPassword().equals(joinForm.getPasswordRepeated())) {
            return ResponseEntity.badRequest().body(Map.of("passwordError", "비밀번호가 일치하지 않습니다."));
        }
        ...
        try {
            ...
            return ResponseEntity.status(HttpStatus.CONFLICT).body(Map.of("error", "동일한 사용자 혹은 이메일이 존재합니다."));
        } catch (Exception e) {
